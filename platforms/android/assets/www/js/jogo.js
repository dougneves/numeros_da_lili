...
const frases_certas = ['Parabéns', 'Muito bem', 'Correto', 'Isso aí', 'Certo']
...
function DesenhaNumero(qual, onde) {
  var n = ('' + qual).split(''),
    i,
    html = ''
  for (i = 0; i < n.length; i++) html += '<img src="images/' + n[i] + '.png">'
  $(onde)
    .attr('data-val', qual)
    .html(html)
...
function GeradorMetodoA() {
  ...
  numero = GeraNumero(
    (jogo.level - 1) * jogo.multiplicador,
    jogo.level * jogo.multiplicador
  )

  jogo.correto_ix = GeraNumero(0, jogo.posicoes)
  if (GeraNumero(0, 2) == 0) {
    for (i = 0; i < jogo.posicoes; i++) jogo.numeros[i] = numero + i
  } else {
    if (numero < jogo.posicoes) numero += jogo.posicoes
    for (i = jogo.posicoes - 1; i >= 0; i--) jogo.numeros[i] = numero - i
  }
}
...
function TestaNumero(e) {
  ...
  if (e.attr('data-val') == jogo.numeros[jogo.correto_ix]) {
    ...
    if (++jogo.acertos % jogo.passar_nivel == 0) jogo.passou = true
  } else {
